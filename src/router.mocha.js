...
function waitResponse(response, raw) {
  return new Promise((resolve, reject) => {
    ...
    response.body.pipe(StreamTest.v2.toText((err, text) => {
      if(err) {
        reject(err);
        return;
      }
...
describe('initHTTPRouter', () => {
  ...
  const httpTransactionStart = sinon.spy(
    buildResponse => buildResponse()
  );
  const httpTransactionCatch = sinon.spy(
    (err) => { throw HTTPError.cast(err); }
  );
  const httpTransactionEnd = sinon.spy();
  const httpTransaction = sinon.spy(
